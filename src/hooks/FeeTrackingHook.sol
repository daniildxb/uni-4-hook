 ...
 */
abstract contract FeeTrackingHook is CustodyHook, RolesHook {
    ...
    function collectFees(address treasury) external virtual onlyHookManager returns (uint128 amount0, uint128 amount1) {
        ...
        unclaimedFees = 0;
        _transferFees(amount0, amount1, treasury);
        lastSeenAssets = totalAssets();
        emit FeesCollected(unclaimedFees, amount0, amount1, treasury);
    }
