...
import {PriorityOrder, PriorityInput, PriorityOutput, PriorityCosignerData} from "@uniswapx/src/lib/PriorityOrderLib.sol";
...
contract ExecutorTest is BaseTest, DeployPermit2, PermitSignature {
    ...
    function _deployPeriphery() internal {
        ...
        executor = new UniswapXExecutor(manager, address(priorityReactor), address(this));
    }
    ...
    function test_fillSwapOrderZeroForOne() public {
        ...
        PriorityOrder memory order = PriorityOrder({
            info: OrderInfoBuilder.init(address(priorityReactor))
                .withSwapper(swapper)
        ...
        UniswapXExecutor.OrderRoutingData memory routingData = UniswapXExecutor.OrderRoutingData({
            poolId: PoolId.unwrap(simplePoolId)
        });
    ...
    function test_fillSwapOrderOneForZero() public {
        ...
        PriorityOrder memory order = PriorityOrder({
            info: OrderInfoBuilder.init(address(priorityReactor))
                .withSwapper(swapper)
                .withDeadline(deadline)
                .withNonce(1), // Use a different nonce
            cosigner: cosigner,
            auctionStartBlock: block.number,
            baselinePriorityFeeWei: 0,
            input: PriorityInput({
                token: ERC20(Currency.unwrap(token1)), 
                amount: inputAmount, 
                mpsPerPriorityFeeWei: 0
            }),
            outputs: outputs,
    ...
    function test_revertWhen_InvalidCaller() public {
        ...
        PriorityOrder memory order = PriorityOrder({
            ...
            baselinePriorityFeeWei: 0,
            input: PriorityInput({
                token: ERC20(Currency.unwrap(token0)), 
                amount: inputAmount, 
                mpsPerPriorityFeeWei: 0
            }),
            outputs: outputs,
